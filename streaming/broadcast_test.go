...
func TestBroadcastNewWithOptionalParams(t *testing.T) {
	...
	err := client.Streaming.Broadcasts.New(context.TODO(), streaming.BroadcastNewParams{
		Broadcast: streaming.BroadcastNewParamsBroadcast{
			Name:               "Broadcast",
			AdID:               gcore.Int(1),
			CustomIframeURL:    gcore.String(""),
			PendingMessage:     gcore.String("pending_message"),
			PlayerID:           gcore.Int(14),
			Poster:             gcore.String("poster"),
			ShareURL:           gcore.String(""),
...
func TestBroadcastUpdateWithOptionalParams(t *testing.T) {
	...
	_, err := client.Streaming.Broadcasts.Update(
		context.TODO(),
		0,
...
func TestBroadcastListWithOptionalParams(t *testing.T) {
	...
	_, err := client.Streaming.Broadcasts.List(context.TODO(), streaming.BroadcastListParams{
		Page: gcore.Int(0),
	})
	if err != nil {
		var apierr *gcore.Error
...
func TestBroadcastGet(t *testing.T) {
	...
	_, err := client.Streaming.Broadcasts.Get(context.TODO(), 0)
	if err != nil {
		var apierr *gcore.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
...
func TestBroadcastGetSpectatorsCount(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Streaming.Broadcasts.GetSpectatorsCount(context.TODO(), 0)
	if err != nil {
		var apierr *gcore.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
