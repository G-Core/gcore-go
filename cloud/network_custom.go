...
// UpdateAndPoll updates a network and polls for completion of the first task,
// returning the updated Network. Use the [TaskService.Poll] method if you need to
// poll for all tasks.
func (r *NetworkService) UpdateAndPoll(ctx context.Context, networkID string, params NetworkUpdateParams, opts ...option.RequestOption) (res *Network, err error) {
	// Exclude WithResponseBodyInto for the action (Update returns TaskIDList, must deserialize properly)
	actionOpts := requestconfig.ExcludeResponseBodyInto(opts...)
	resource, err := r.Update(ctx, networkID, params, actionOpts...)
	if err != nil {
		return
	}
	...
	// Clear request body for Get
	getOpts := slices.Concat(opts, []option.RequestOption{requestconfig.WithoutRequestBody()})
	return r.Get(ctx, networkID, getParams, getOpts...)
}
