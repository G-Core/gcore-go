...
type BillingReservationListParams struct {
	// Start of the reservation period (ISO 8601). The period must not exceed 31 days.
	// The API returns monthly pricing for this period.
	TimeFrom time.Time `query:"time_from" api:"required" format:"date-time" json:"-"`
	// End of the reservation period (ISO 8601). The period must not exceed 31 days.
	// The API returns monthly pricing for this period.
	TimeTo time.Time `query:"time_to" api:"required" format:"date-time" json:"-"`
