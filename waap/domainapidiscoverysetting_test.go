...
func TestDomainAPIDiscoverySettingUpdateWithOptionalParams(t *testing.T) {
...
func TestDomainAPIDiscoverySettingGet(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Waap.Domains.APIDiscovery.Settings.Get(context.TODO(), 1)
	if err != nil {
		var apierr *gcore.Error
		if errors.As(err, &apierr) {
			t.Log(string(apierr.DumpRequest(true)))
