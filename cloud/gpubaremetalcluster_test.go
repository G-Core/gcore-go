...
func TestGPUBaremetalClusterAction(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Cloud.GPUBaremetal.Clusters.Action(
		...
		cloud.GPUBaremetalClusterActionParams{
			ProjectID: gcore.Int(1),
			RegionID:  gcore.Int(7),
			OfStart:   &cloud.GPUBaremetalClusterActionParamsBodyStart{},
		},
	)
	if err != nil {
		var apierr *gcore.Error
...
func TestGPUBaremetalClusterApplySettingsWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Cloud.GPUBaremetal.Clusters.ApplySettings(
		context.TODO(),
		"1aaaab48-10d0-46d9-80cc-85209284ceb4",
