...
// QuotaRegionalService contains methods and other services that help with
// interacting with the gcore API.
//
// Note, unlike clients, this service does not read variables from the environment
// automatically. You should not instantiate this service directly, and instead use
// the [NewQuotaRegionalService] method instead.
type QuotaRegionalService struct {
	Options []option.RequestOption
}

// NewQuotaRegionalService generates a new service that applies the given options
// to each request. These options are applied after the parent client's options (if
// there is one), and before any request-specific options.
func NewQuotaRegionalService(opts ...option.RequestOption) (r QuotaRegionalService) {
	r = QuotaRegionalService{}
	r.Options = opts
	return
}

// Get a quota by region
func (r *QuotaRegionalService) Get(ctx context.Context, clientID int64, query QuotaRegionalGetParams, opts ...option.RequestOption) (res *RegionalQuotas, err error) {
...
func (r RegionalQuotas) RawJSON() string { return r.JSON.raw }
func (r *RegionalQuotas) UnmarshalJSON(data []byte) error {
	return apijson.UnmarshalRoot(data, r)
}

type QuotaRegionalGetParams struct {
	// Region ID
	//
...
func (f QuotaRegionalGetParams) IsPresent() bool { return !param.IsOmitted(f) && !f.IsNull() }
