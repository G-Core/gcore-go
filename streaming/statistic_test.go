...
func TestStatisticGetMeetSeriesWithOptionalParams(t *testing.T) {
	baseURL := "http://localhost:4010"
	if envURL, ok := os.LookupEnv("TEST_API_BASE_URL"); ok {
		baseURL = envURL
	}
	...
	_, err := client.Streaming.Statistics.GetMeetSeries(context.TODO(), streaming.StatisticGetMeetSeriesParams{
		From:        "from",
		To:          "to",
		Granularity: streaming.StatisticGetMeetSeriesParamsGranularity1m,
	})
	if err != nil {
		var apierr *gcore.Error
		if errors.As(err, &apierr) {
